library instagram_page_indicator;
...
class InstagramPageIndicator extends StatefulWidget {
  ...
  const InstagramPageIndicator({
    Key? key,
    this.dotSize = 6.0,
    this.dotSelectedSize = 8.0,
    this.dotColor = Colors.grey,
    this.dotSelectedColor = Colors.blueGrey,
    this.dotSpacing = 13.0,
  ...
  State<StatefulWidget> createState() {
    return _InstagramPageIndicatorState();
  }
}

class _InstagramPageIndicatorState extends State<InstagramPageIndicator> {
  ...
  @override
  void didUpdateWidget(InstagramPageIndicator oldWidget) {
  ...
  Widget build(BuildContext context) {
    int itemCount =
        widget.itemCount >= widget.visibleDotCount ? widget.visibleDotCount : widget.itemCount;
    ...
    Widget child = SizedBox(
      ...
      child: CustomPaint(painter: _Painter(widget, currentPage, _paint, widget.orientation)),
    );
...
class _Painter extends CustomPainter {
  final InstagramPageIndicator _widget;
  final double _page;
  final Paint _paint;
  final Axis orientation;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double scaleDistance =
        (_widget.dotSpacing + (_widget.dotSelectedSize - _widget.dotSize) / 2) * 0.7;
    double smallScaleDistance = _widget.dotSelectedSize / 2;

    int firstVisibleDotPos =
        ((_visibleFramePosition - _firstDotOffset) / _widget.dotSpacing).floor();
    int lastVisibleDotPos = firstVisibleDotPos +
        ((_visibleFramePosition + width - getDotOffsetAt(firstVisibleDotPos)) / _widget.dotSpacing)
            .floor();
